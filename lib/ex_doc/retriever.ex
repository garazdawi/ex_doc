defmodule ExDoc.Retriever do
  ...
  defp get_function(
    ...
    group = GroupMatcher.match_function(groups_for_docs, Map.merge(%{kind: type, name: name,
                                                                     arity: arity, module: module_data.module}, metadata))
  ...
  defp get_callback(callback, source, groups_for_docs, module_data, annotations_for_docs) do
    ...
    group = GroupMatcher.match_function(groups_for_docs, Map.merge(%{kind: kind, name: name,
                                                                     arity: arity, module: module_data.module},metadata))
  ...
  defp get_type(type_entry, source, groups_for_docs, module_data, annotations_for_docs) do
    {:docs_v1, _, _, content_type, _, module_metadata, _} = module_data.docs
    {{kind, name, arity}, anno, _signature, doc, metadata} = type_entry
    doc_line = anno_line(anno)
    source = anno_file(anno, source)
